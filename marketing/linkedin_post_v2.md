...
```
ACTIVE (Agent A working)
  ↓
Agent B checks → HIGH_RISK
  ↓
LOCKED (decision point for Agent B)
  ↓
Agent B chooses WAIT
  ↓
WAITING (Agent B checkpoint saved, event-subscribed)
  ↓
Agent A: COMPLETED (logs completion)
  ↓
System: lock_removed event fires
  ↓
Agent B: RESUMED (wakes from sleep, continues)
```
